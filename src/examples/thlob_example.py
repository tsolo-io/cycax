...
class Thing(Cuboid):
    ...
    def calculate(self):
        ...
        self.make_rectangle(side="FRONT", x=self.x_size/3, y=5, x_size=(self.x_size)/3, y_size=self.y_size/3 +2, z_size=self.z_size-10, sink = -1)
        self.make_rectangle(side="LEFT", x=self.x_size/3, y=5, y_size=(self.x_size)/3, x_size=self.y_size/3 +2, z_size=self.z_size-10, sink = -1)
